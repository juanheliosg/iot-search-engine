...
class ExtractorActorTest extends ScalaTestWithActorTestKit(ExtractorTestSpec.config)
  ...
  val extData: ExtractorState = ExtractorState(schema,config,ExtractorType.Http)
  ...
  "Guardian extractor actor" must  {
    ...
    "update extractor from running state" in {
      ...
      val schema = new DataSchema(
        newSourceId, "ayto:idSensor", "dc:modified",
        List(new Measure("ocupation","ayto:ocupacion",2),
          new Measure("intensity", "ayto:intensidad",1))
      )
      ...
      val extData2 = ExtractorState(schema,config, ExtractorType.Http)
      eventSourcedTestKit.runCommand[StatusReply[Summary]](ref => ExtractorGuardianEntity.updateExtractor(extData2, ref))

      val result = waitUntilStarted

      result.stateOfType[ExtractorGuardianEntity.RunningExtractor].state shouldBe "running"
      result.stateOfType[ExtractorGuardianEntity.RunningExtractor].extractorState.schema.sourceID shouldBe newSourceId
    }
